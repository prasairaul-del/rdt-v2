...
export async function filePickerAgent(
  ...
  try {
    ...
    if (!listResult.success || !listResult.data) {
      return {
        success: false,
        error: { message: 'Failed to list project files', code: 'LIST_FILES_FAILED', recoverable: true },
        modelUsed: 'none',
        providerUsed: 'none',
        toolCalls,
      };
    ...
    for (const term of searchTerms.slice(0, 5)) {
      ...
      const searchResult = await searchFilesTool.execute({ pattern: term, maxResults: 10 });
    ...
    const keyFiles = [
      ...
      'AGENTS.md', 'knowledge.md', '.rdt/config.yaml',
    ];
    ...
    if (projectRoot) {
      try {
        ...
        errors.push(`Vector search failed: ${err instanceof Error ? err.message : String(err)}`);
      }
    }
    ...
    for (const file of topFiles) {
      ...
      const readResult = await readFileTool.execute({ path: file.path, maxBytes: 8_000 });
      ...
      if (!readResult.success) {
        errors.push(`Could not read ${file.path}: ${readResult.error?.message ?? 'unknown'}`);
      }
    }
...
function extractSearchTerms(request: string): string[] {
  ...
  for (const word of words) {
    ...
    if (['the', 'this', 'that', 'with', 'from', 'fix', 'add', 'make', 'need', 'want', 'should'].includes(clean.toLowerCase())) continue;
...
function scoreFiles(
  ...
  const scored: Array<{ path: string; score: number; reason: string; priority: string }> = [];
  ...
  for (const file of allFiles) {
    ...
    // Boost config files
    if (file.endsWith('.json') || file.endsWith('.yaml') || file.endsWith('.yml') || file.endsWith('.toml')) {
      score += 1;
    }
    ...
    if (ext === 'ts' || ext === 'tsx' || ext === 'js' || ext === 'jsx') score += 2;

    if (score > 0) {
      scored.push({
        path: file,
        score,
        reason: reasonParts.length > 0 ? reasonParts.slice(0, 2).join(', ') : 'source file',
