...
import { errorResult, successResult } from '../../src/core/result';
...
beforeAll(() => {
  ...
  writeFileSync(
    join(TEST_TMP, 'hello.txt'),
    'Hello, World!\nSecond line\nThird line\n',
  );
  writeFileSync(
    join(TEST_TMP, 'config.json'),
    JSON.stringify({ key: 'value', nested: { num: 42 } }, null, 2),
  );
  mkdirSync(join(TEST_TMP, 'subdir'), { recursive: true });
  writeFileSync(
    join(TEST_TMP, 'subdir', 'nested.txt'),
    'Nested file content\n',
  );
});
...
describe('searchFilesTool', () => {
  it('should find files by filename match', async () => {
    ...
    expect(result.success).toBe(true);
    const paths =
      result.data?.results.map((r) => r.path.replace(/\\/g, '/')) ?? [];
