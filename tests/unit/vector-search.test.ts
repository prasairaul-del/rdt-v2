    ...
    query(sql: string) {
      ...
      return {
        ...
        all: () => {
          ...
          if (sql.includes('SELECT path, terms_freq')) {
            return Array.from(store.values()).map((v) => ({
              path: v.path,
              terms_freq: v.terms_freq,
            }));
          }
...
describe('VectorSearch Local Search Engine', () => {
  ...
  it('should initialize SQLite cache and index files via TF-IDF', async () => {
    ...
    const multResults = await vectorSearch.search(
    ...
    const divResults = await vectorSearch.search(
      'division quotient denominator',
    );
    ...
    const readmeResults = await vectorSearch.search(
      'setup instructions install tests',
    );
  ...
  it('should leverage dense embeddings when ProviderRouter is configured', async () => {
    const testConfig: RdtConfig = {
      ...
      model_policies: {
        cheap_fast: {
          prefer: ['mock-provider/text-embedding-3-small'],
          max_cost: 'low',
        },
      },
      agents: {},
    ...
    const results = await vectorSearch.search(
      'any query string for testing embeddings',
    );
