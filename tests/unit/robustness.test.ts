...
const testConfig: RdtConfig = {
  ...
  model_policies: {
    cheap_fast: {
      prefer: ['openrouter-1/free', 'openrouter-2/free'],
      max_cost: 'high',
    },
    smart_reasoning: {
      prefer: ['openrouter-1/free', 'openrouter-2/free'],
...
describe('TaskRunner Robustness & Failures', () => {
  it('should fall back to a secondary provider if the primary provider goes offline mid-task', async () => {
    ...
    const provider1 = new MockProvider('openrouter-1', {
      enabled: true,
      models: [
        {
          id: 'free',
    ...
    provider1.complete = async (request) => {
      ...
      throw Object.assign(new Error('Provider went offline'), {
        code: 'SERVER_ERROR',
        retryable: false,
      });
    };
    ...
    const provider2 = new MockProvider('openrouter-2', {
      enabled: true,
      models: [
        {
          id: 'free',
    ...
    const plannerCall = result.state.providerUsage.find(
      (u) => u.agentName === 'planner',
    );
    const editorCall = result.state.providerUsage.find(
      (u) => u.agentName === 'editor',
    );
  ...
  it('should cool down a provider on rate limiting (429) and route subsequent requests past it', async () => {
    ...
    const plannerUsage = result.state.providerUsage.filter(
      (u) => u.agentName === 'planner',
    );
    ...
    const editorUsage = result.state.providerUsage.filter(
      (u) => u.agentName === 'editor',
    );
