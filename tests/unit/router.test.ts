...
import {
  ...
  checkCooldown,
  filterCooldown,
} from '../../src/router/cooldown';
import {
  type RouterRequirements,
  hasCapability,
  matchModels,
} from '../../src/router/model-policy';
import { ProviderRouter } from '../../src/router/provider-router';
import {
  checkRateLimit,
  filterRateLimited,
} from '../../src/router/rate-limit-state';
...
describe('model-policy', () => {
  ...
  it('should filter by max_cost', () => {
    ...
    const result = matchModels(policy, mixedModels, {
      needsTools: false,
      needsJson: false,
    });
    // Only free and low cost models
    expect(result.every((m) => m.cost === 'free' || m.cost === 'low')).toBe(
      true,
    );
  });
  ...
  it('should sort by preference order', () => {
    const policy = {
      prefer: ['groq/fast', 'openrouter/*'],
      max_cost: 'high' as const,
    };
    const result = matchModels(policy, mixedModels, {
      needsTools: false,
      needsJson: false,
    ...
    const openrouterIdx = result.findIndex(
  ...
  it('should filter out models lacking required JSON support', () => {
    ...
    const policy = { prefer: ['*/*'], max_cost: 'high' as const };
    const result = matchModels(policy, modelsWithJsonFalse, {
      needsTools: false,
      needsJson: true,
    });
    expect(result.some((m) => m.providerId === 'groq')).toBe(false);
  ...
  it('should return empty array when no models match cost ceiling', () => {
    ...
    const result = matchModels(policy, expensiveModels, {
      needsTools: false,
      needsJson: false,
    });
...
describe('cooldown', () => {
  ...
  it('filterCooldown should separate available from cooling models', () => {
    const future = new Date(Date.now() + 60_000).toISOString();
    const models = [{ cooldownUntil: undefined }, { cooldownUntil: future }];
...
describe('ProviderRouter', () => {
  const testConfig: RdtConfig = {
    ...
    context_budget: {
      ...
      file_picker_max_tokens: 2_000,
      planner_max_tokens: 4_000,
      editor_max_tokens: 8_000,
      reviewer_max_tokens: 4_000,
      max_file_read_tokens: 10_000,
      max_total_file_tokens_per_step: 40_000,
  ...
  it('should create router and register providers', () => {
    ...
    const provider1 = new MockProvider('openrouter', {
      enabled: true,
      models: [
        {
          id: 'free',
    ...
    const provider2 = new MockProvider('groq', {
      enabled: true,
      models: [
        {
          id: 'fast',
  ...
  it('should skip to next provider if preferred model fails', async () => {
    const router = new ProviderRouter(testConfig);
    const failingProvider = new MockProvider('openrouter', {
      enabled: true,
      models: [
        {
          id: 'free',
    ...
    const goodProvider = new MockProvider('groq', {
      enabled: true,
      models: [
        {
          id: 'fast',
    ...
    expect(result.attempts.filter((a) => a.status === 'success')).toHaveLength(
      1,
    );
    expect(result.success).toBe(true);
    expect(result.response).toBeDefined();
  ...
  it('should track attempts and exhausted models on total failure', async () => {
    const router = new ProviderRouter(testConfig);
    const failing1 = new MockProvider('openrouter', {
      enabled: true,
      models: [
        {
          id: 'free',
    ...
    const failing2 = new MockProvider('groq', {
      enabled: true,
      models: [
        {
          id: 'fast',
    ...
    failing2.complete = async () => {
      throw Object.assign(new Error('fail too'), {
        code: 'SERVER_ERROR',
        retryable: false,
      });
    };
