...
bun run test         # Run 227 tests across 11 suites
...
- **5 CLI commands** — `init`, `status`, `run`, `providers`, `dashboard` — built with Commander.
- **Interactive Local Dashboard Web UI** — A beautiful dark-mode glassmorphic interface styled with custom Vanilla CSS variables, displaying real-time task nodes, cost estimators, token trackers, and side-by-side parsed code diffs.
- **Task Console and Concurrency Lock** — Allows triggering coding tasks directly from the dashboard UI, backed by a single-task background execution lock to prevent repository write conflicts.
...
| `unit/cli.test.ts` | 15 | --help, --version, init, status, dashboard options |
